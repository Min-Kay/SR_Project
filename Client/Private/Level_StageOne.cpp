...
HRESULT CLevel_StageOne::Ready_Layer_Map()
{
	...
	Lefttrans->Set_State(CTransform::STATE_POSITION, _float3(10.5f, 2.f, 0.7f));
	}
	// Start Right Door
	{
		...
		Righttrans->Set_State(CTransform::STATE_POSITION, _float3(13.5f, 2.f, 0.7f));
	}
	...
	// 입구 바닥 충돌체
	{
		...
		box->Set_State(CBoxCollider::COLL_SIZE, _float3(7.f, 1.f, 6.f));
		box->Set_AdditionalPos(_float3((iWaterZ * 0.5f), 1.f, (-iWaterZ * 0.5f)));
	...
	//뒷면 충돌체_1
	{
		...
		transform->Set_State(CTransform::STATE_POSITION, _float3((iWaterZ * 3) + 2, 0.f, 1.f));
	...
	// 뒷면 충돌체_2
	{
		...
		transform->Set_State(CTransform::STATE_POSITION, _float3((iWaterZ * 2), 4.f, 1.f));
	...
	for (_int i = 0; i < iLineY; ++i)
	{
		for (_int j = 0; j < iLineX; ++j)
		{
			if (FAILED(pGameInstance->Add_GameObject(LEVEL_STAGEONE, TEXT("Layer_Front"), TEXT("Prototype_GameObject_Tile"))))
				return E_FAIL;
			CGameObject* BackTile = pGameInstance->Get_GameObject(LEVEL_STAGEONE, TEXT("Layer_Front"), (i * iLineX) + j);
			CTransform* BackTrans = (CTransform*)BackTile->Get_Component(COM_TRANSFORM);

			BackTrans->Set_State(CTransform::STATE_POSITION, _float3((j * 1.f), (i * 1.f), (_float)iLineZ - 1));
		}
	}

	// 정면 충돌체
	{
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_STAGEONE, TEXT("Layer_F_Collider"), TEXT("Prototype_GameObject_TileCollider"))))
			return E_FAIL;

		CGameObject* tileCollider = pGameInstance->Get_GameObject(LEVEL_STAGEONE, TEXT("Layer_F_Collider"), 0);
		// tile의 위치
		CTransform* transform = (CTransform*)tileCollider->Get_Component(COM_TRANSFORM);
		static_cast<CTileCollider*>(tileCollider)->Set_TextureIndex(0);
		// collider의 위치
		...
		box->Set_State(CBoxCollider::COLL_SIZE, _float3((_float)iLineX, (_float)iLineY, 0.1f));
		box->Set_AdditionalPos(_float3((iLineX * 0.5f), iLineY * 0.5f, 0.f));
	...
	// 오른쪽면 충돌체 big problem 충돌은 되는데 포탈 찍히는게 타일 밖으로 보임
	{
		...
		transform->Set_State(CTransform::STATE_POSITION, _float3((_float)iLineX - 1, 0.f, 0.f));

		box->Set_State(CBoxCollider::COLL_SIZE, _float3(1.f, (_float)iLineY, (_float)iLineZ));
		box->Set_AdditionalPos(_float3(0.f, iLineY * 0.5f, iLineZ * 0.5f));
	...
	// 바닥면 충돌체_0
	{
		...
		transform->Set_State(CTransform::STATE_POSITION, _float3(0.f, 1.f, 0.f));
	...
	//		static_cast<CTile*>(BottomTile)->Set_TextureIndex(1);
	//		BottomTrans->Rotation(_float3(1.f, 0.f, 0.f), D3DXToRadian(90));
	//		BottomTrans->Set_State(CTransform::STATE_POSITION, _float3((j * 1.f), -1.f, (iWaterZ * 2) + (i * 1.f)));
	...
	//			CGameObject* Water = pGameInstance->Get_GameObject(LEVEL_STAGEONE, TEXT("Layer_Water"), (i * iLineX) + j);
	//			CTransform* WaterTrans = (CTransform*)Water->Get_Component(COM_TRANSFORM);
	//			WaterTrans->Set_State(CTransform::STATE_POSITION, _float3((j * 1.f), 0.f, (iWaterZ * 2) + (i * 1.f)));
	...
	// 바닥면 충돌체_2
	{
		...
		transform->Set_State(CTransform::STATE_POSITION, _float3(0.f, 1.f, (iWaterZ * 3) + 0.f));

		box->Set_State(CBoxCollider::COLL_SIZE, _float3((_float)iLineX, 1.f, (_float)iLineZ));
		box->Set_AdditionalPos(_float3(iLineX * 0.5f, 0.f, iWaterZ * 0.5f * 3));
	...
	static_cast<CTile*>(BottomTile1)->Set_TextureIndex(1);
